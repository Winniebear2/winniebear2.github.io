//1. 动态生成div和imgCell填充整个html框架
//2. 对于图片或内容随机并能复原的思想： 通过数组，记录位置；打乱数组，形成随机位置； 用原先的数组指导位置；
//3. 数组乱序的方法，注意sort排序会对原数组有影响，因此用slice剪切一个全新数组，不让其影响原来数组
//4. 拖拽三步： ① MouseDown获取当前鼠标位置(e.pageX  e.pageY) ② MouseMove跟踪鼠标位置并动态设置div的left top值 ③ MouseUp抬起
//   如果要实现拖拽到某个盒子中，要设置和比较盒子的offset().top 以及 offset().left
//5. 移动后需要判定是交换位置还是返回原来的位置 通过判定MouseUp位置来进行判定
